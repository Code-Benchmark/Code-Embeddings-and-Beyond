...
identifier_type = {
    ...
    'java': ['identifier', 'type_identifier', 'scoped_identifier', 'scoped_type_identifier', 'enum_constant'],
    'c': ['identifier','type_identifier','statement_identifier', 'field_identifier']
}
string_type = {
    ...
    'java': ['comment', 'string_literal', 'character_literal', 'annotation', 'marker_annotation'],
    'c': ['string_literal', 'concatenated_string', 'system_lib_string', 'escape_sequence', 'char_literal', 'comment']
}
...
def clean_convert_split(args, paths, code): #, f_name):
    ...
    for idx, path in enumerate(paths):
        ...
        if terminal.type in string_type[args.language]:
            ...
            #     get_func = True
            if terminal.type in identifier_type[args.language]:
                for block in blocks:
                    ...
                    temp_paths.append(path + [new_node])
                                                         ...
                                                         'hex_floating_point_literal', 'number_literal']:
...
def language_parse(args, data, lang_parser):
    ...
    return path_pool, [node.type for node in terminals], [int(node.named) for node in terminals], data['label'], \
...
def sub_process(args, idx, all_data, lang_parser):
    ...
    # source_dic = json.loads('../data/c/source_vocab.json')
    with open(save_path, 'w') as f:
        for data in tqdm(all_data):
            ...
            paths, code_tokens, code_named, label, paths_map, row, r_path_idx, r_paths = \
                language_parse(args, data, lang_parser)
            # token_statistic(source_dic, target_dic, code_tokens, func_name)
            token_statistic(source_dic, code_tokens)
            data = {'target': label, #func_name,
...
def compress(data):
    ...
    s = str(target) + '\t' + "|".join([word for word in content]) + '\t' \
...
def process(args):
    ...
    source_dic, target_dic = dict(), dict()
    with open('../data/c/source_vocab.json', 'r') as f:
        saved_source_dic = json.loads(f.readline())
        for key, value in saved_source_dic.items():
    ...
    for i in range(args.process_num):
        ...
        try:
            with open(sub_dict_path, 'r') as f:
                ...
                update_sum_dict(sub_source_dic, source_dic)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--language', choices=['python', 'javascript', 'ruby', 'go','c','java'], type=str, default='c')
