...
from model.model_modify import Model
import torch
import numpy as np
import random
...
def train():
    ...
    # dataset
    parser.add_argument("--dataset", type=str, help="train dataset", default='c',
                        choices=['python', 'ruby', 'javascript', 'go', 'c', 'java'])
    parser.add_argument("--on_memory", type=boolean_string, default=True, help="Loading datasets into memory")

    # dataset size
    parser.add_argument("--max_code_length", type=int, default=1000, help="")
    ...
    parser.add_argument("--vocab_threshold", type=int, default=3, help="not work for ct_vocab")
    ...
    parser.add_argument("--val_batch_size", type=int, default=32, help="number of batch_size of valid")
    ...
    # transformer
    # path_embedding_size * attn_heads = hidden
    ...
    if args.data_debug:
        ...
        train_dataset = PathAttenDataset(args, s_vocab, type_='valid')
    else:
        train_dataset = PathAttenDataset(args, s_vocab, type_='train')

    print("Loading Valid Dataset")
    # valid_dataset = PathAttenDataset(args, s_vocab, type_='valid')
    print("Loading Test Dataset")
    test_dataset = PathAttenDataset(args, s_vocab, type_='test')
    if args.on_memory:
    ...
    trainer = Trainer(args=args, model=model, train_data=train_data_loader,
                      test_data=test_data_loader) #valid_data=valid_data_loader,
    #, valid_data=valid_data_loader,test_data=test_data_loader
    ...
    for epoch in range(0, args.epochs):
        # if epoch == 0:
        #     trainer.test(epoch,'test')
        #     continue
        if args.train:
            trainer.train(epoch)
        if args.test:
            # trainer.test(epoch,'valid')
            trainer.test(epoch,'test')
