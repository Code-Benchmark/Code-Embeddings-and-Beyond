...
class PSCSNetwork(nn.Module):
    def __init__(self, config, nl_vocab_len, path_vocab_len, device, pretrained_weight=None):
    ...
    def init_weights(self):
        ...
        if self.pretrained_weight is not None:
            ...
            pretrained_weight = np.concatenate((self.nl_emb.weight.data[0:5], self.pretrained_weight), axis=0)
            self.nl_emb.weight.data.copy_(torch.from_numpy(pretrained_weight))
    ...
    def forward(self, nl, path, start_token, end_token, epoch=0):
        ...
        cos = F.cosine_similarity(nl_vec, code_vec).view(batch_size,-1)
