...
from model import *
from model.model_diy import BatchProgramCC
from model.model import PSCSNetwork
...
def train(args):
    word2vec = Word2Vec.load("./data/w2v/node_wiz_token_w2v_128").wv
    MAX_TOKENS = word2vec.syn0.shape[0]
    EMBEDDING_DIM = word2vec.syn0.shape[1]
    embeddings = np.zeros((MAX_TOKENS, EMBEDDING_DIM), dtype="float32")
    # embeddings[5:] = word2vec.syn0
    ...
    model = PSCSNetwork(args, len(nl_vocab), len(path_vocab), device, embeddings)
    # model = BatchProgramCC(args, len(nl_vocab), len(path_vocab), device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (data, trg_data) in data_iter:
            ...
            if i % args.val_freq == 0:
                ...
                f = open('{}/pscs_performance_wiz_w2v.txt'.format(args.save_dir), 'a')
                post_fix['epoch'] = epoch
                f.write(str(post_fix) + '\n')
                f.close()

        scheduler.step()
        model_save_dir = '{}/pscs_model_wiz_w2v'.format(args.save_dir)
