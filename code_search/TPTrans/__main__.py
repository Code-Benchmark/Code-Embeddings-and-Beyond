...
from model.model_diy import Model
import torch
import numpy as np
import random
...
def train():
    ...
    # dataset
    parser.add_argument("--dataset", type=str, help="train dataset", default='java',
                        choices=['python', 'ruby', 'javascript', 'go','java','java_small'])
    parser.add_argument("--on_memory", type=boolean_string, default=False, help="Loading datasets into memory")

    # dataset size
    parser.add_argument("--max_code_length", type=int, default=600, help="")
    parser.add_argument("--max_doc_length", type=int, default=20, help="")
    ...
    parser.add_argument("--vocab_threshold", type=int, default=1, help="not work for ct_vocab")
    ...
    parser.add_argument("--val_batch_size", type=int, default=1, help="number of batch_size of valid")
    parser.add_argument("--infer_batch_size", type=int, default=1, help="number of batch_size of infer")
    ...
    parser.add_argument("--shuffle", type=boolean_string, default=False, help="whether to shuffle the training data")

    # glove
    parser.add_argument("--pretrain", type=boolean_string, default=True,
                        help="Whether to use the glove pretrain embedding")
    parser.add_argument("--embedding_file", type=str, default='data/java/tokens_all_256', help="file path to glove txt")
    ...
    parser.add_argument("--gru_size", type=int, default=32, help="for path gru")
    parser.add_argument("--gru_layers", type=int, default=1, help="for path gru")

    # transformer
    parser.add_argument("--embedding_size", type=int, default=256, help="hidden size of transformer model")
    parser.add_argument("--query_embedding_size", type=int, default=256, help="hidden size of transformer model")
    ...
    parser.add_argument("--pointer", type=boolean_string, default=False, help="whether to use pointer network")
    ...
    parser.add_argument("--train", type=boolean_string, default=False, help="Whether to train")
    ...
    parser.add_argument("--checkpoint", type=str, default='relation_java_2021-12-21-10-03-56_11.pth', help="the checkpoint file path")
    ...
    test_data_loader = DataLoader(test_dataset, batch_size=args.val_batch_size, num_workers=num_workers,
                                        collate_fn=collect_fn)
    print("Building Model")
    model = Model(args, s_vocab, t_vocab)

    print("Creating Trainer")
    trainer = Trainer(args=args, model=model, test_data=test_data_loader, t_vocab=t_vocab) #train_data=train_data_loader,
