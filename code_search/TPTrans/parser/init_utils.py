...
def count_dict_init():
    ...
    keys = ['tokens', 'uni_paths', 'paths', 'named', 'label', 'nums', 'uni_r_paths', 'max_row', 'path_len']
    for k in keys:
        count_dic[k] = 0
    return count_dic
...
def read_files(language, type):
    ...
    dir_path = os.path.join('../raw_data', language)
    file_list = []
    ...
    for file in tqdm(file_list):
        lines = pd.read_pickle(os.path.join(dir_path, file))
        if type == 'valid':
            lines = lines[lines["partition"].isin(['dev'])]
        else:
            lines = lines[lines["partition"].isin([type])] 
        i = 0
        for index,row in lines.iterrows():
            if i == 10:
                break
            i += 1
            data = {'code': row['code'],# 'func_name': split_func_name(row['func_name']),
                    'label': row['label']}
