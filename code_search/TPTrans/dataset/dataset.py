...
class PathAttenDataset(Dataset):
    ...
    def __init__(self, args, s_vocab, type_):
    # def __init__(self, args, s_vocab, t_vocab, type_):
        ...
        self.args = args
        self.type_ = type_
        assert type_ in ['train', 'test', 'valid','small_test']
        if self.on_memory:
    ...
    def __getitem__(self, item):
        ...
        return {key: value if torch.is_tensor(value) or isinstance(value, dict) or key=='origin_r_paths' else torch.tensor(value) for key, value
                in sample.items()}
    ...
    def process(self, data):
        ...
        data_dic = {'content': content_, 'content_mask': content_mask_, 'label': data['target']-1,
                    'path_map': paths_map_, 'paths': paths_, 'paths_mask': paths_mask_, 'named': named_, 'row': row_,
                    'r_paths': r_paths_, 'r_path_idx': r_path_idx_, 'r_paths_mask': r_paths_mask_, 'origin_r_paths':data['r_paths']}
        if self.args.pointer:
...
def collect_fn(batch):
    ...
    data['label'] = torch.stack([b['label'] for b in batch], dim=0)#[:, :max_target_len]
    ...
    data['origin_r_paths'] = sample['origin_r_paths']
    if 'e_voc' in batch[0]:
