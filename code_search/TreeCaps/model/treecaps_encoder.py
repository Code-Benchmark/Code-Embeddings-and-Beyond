...
import numpy as np
from Search.TreeCaps.model.utils import softmax,reduce_sum
...
# def coding_layer(parent_emb, nodes, W_c1, W_c2):
#
#     parent = torch.matmul(parent_emb, W_c1)
#     child_emb = torch.tanh(nodes)
#     # children = torch.matmul(child_emb, W_c2)
#
#     # return torch.cat( (parent,children),2 )
#     return torch.cat((parent, parent), 2)
def squash(vector):
...
def dynamic_routing(shape, input, W, biases, num_outputs=10, num_dims=16):
    ...
    return (v_J)
class treecaps_encoder(nn.Module):
    def __init__(self,n_vol, feature_size, label_size, conv_feature, cov_size, num_cov, train_num_feats, num_dims, num_outputs, Wemd, pretrained_weight):
        super(treecaps_encoder, self).__init__()
        ...
        self.dr_w = torch.nn.Parameter(
            torch.nn.init.normal(torch.zeros((1, 576, num_dims * num_outputs, 8, 1), dtype=torch.float32, device=cuda1),
                                 std=0.01))
        self.dr_b = torch.nn.Parameter(torch.randn((1, 1, num_outputs, num_dims, 1), dtype=torch.float32, device=cuda1))
        if pretrained_weight is not None:
            self.embed = torch.nn.Embedding(n_vol+1, feature_size)
        ...
        w_h = truncated_normal_(torch.zeros((16, 128)), std=1.0 / math.sqrt(16)).cuda()
        b_h = truncated_normal_(torch.zeros(128, ), std=math.sqrt(2.0 / 16)).cuda()
    ...
    def hidden_layer(self,codeCaps):

        return torch.tanh(torch.matmul(codeCaps, self.w_h) + self.b_h)
    ...
    def encode(self, nodes, children):
        ...
        codeCaps = torch.squeeze(torch.squeeze(torch.squeeze(codeCaps, axis=1),axis=1),axis=2)
        hidden = self.hidden_layer(codeCaps)
