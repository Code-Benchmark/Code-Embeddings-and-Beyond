...
import numpy as np
from TreeCaps.classifier.  utils import softmax,reduce_sum
    ...
    child_indices = torch.mul(
        (torch.unsqueeze(
            torch.unsqueeze(
                ...
                torch.arange(-1.0, torch.as_tensor(max_children, dtype=torch.float32),1.0, dtype=torch.float32),
                dim=0),
...
def loss_layer(logits_node,labels):
    ...
    # labels = Variable(torch.randn(N, label_size,).type(dtype), requires_grad=False)

    max_l = torch.square(torch.maximum(torch.as_tensor(0.,device=cuda1), 0.9 - logits_node))
    max_r = torch.square(torch.maximum(torch.as_tensor(0.,device=cuda1), logits_node - 0.1))
    T_c = labels
    L_c = T_c * max_l + 0.5 * (1 - T_c) * max_r

    loss = torch.mean(torch.sum(L_c, axis=1))
...
class Treecaps(nn.Module):
    def __init__(self, n_vol,feature_size, label_size, conv_feature, cov_size, num_cov, train_num_feats, num_dims, num_outputs, tlen,Wemd):
        ...
        self.w_t = torch.nn.Parameter(torch.nn.init.trunc_normal_(torch.zeros((train_num_feats, conv_feature), device=cuda1),
                                std=0.01))
        self.w_l = torch.nn.Parameter(torch.nn.init.trunc_normal_(torch.zeros((train_num_feats, conv_feature), device=cuda1),
                                std=0.01))
        self.w_r = torch.nn.Parameter(torch.nn.init.trunc_normal_(torch.zeros((train_num_feats, conv_feature), device=cuda1),
                                std=0.01))
        self.init = torch.nn.init.trunc_normal_(torch.zeros(conv_feature, ), std=math.sqrt(2.0 / train_num_feats))
        ...
        self.dr_w = torch.nn.Parameter(
            torch.nn.init.normal(
                torch.zeros((1, 1280, num_dims * num_outputs, 8, 1), dtype=torch.float32, device=cuda1),
                std=0.01))
        ...
        # self.embed.weight.data.copy_(torch.from_numpy(weight['node_type']))
        self.embed.weight.requires_grad = True
        # self.w_h = torch.nn.Parameter(truncated_normal_(torch.zeros((104, tlen),device=cuda1), std=1.0 / math.sqrt(conv_feature)))
        # self.b_h = torch.nn.Parameter(truncated_normal_(torch.zeros(tlen,device=cuda1), std=math.sqrt(2.0 / conv_feature)))
        # self.emb = torch.nn.Parameter(self.Wemd)
        ...
        self.caps1_out_caps = 104 #label size
    ...
    def forward(self, nodes, children):
        ...
        conv = [
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t, self.w_l,self.w_r, self.b_conv),dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_1, self.w_l_1, self.w_r_1, self.b_conv_1),
                            dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_2, self.w_l_2, self.w_r_2, self.b_conv_2),
                            dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_3, self.w_l_3, self.w_r_3, self.b_conv_3),
                            dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_4, self.w_l_4, self.w_r_4, self.b_conv_4),
                            dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_5, self.w_l_5, self.w_r_5, self.b_conv_5),
                            dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_6, self.w_l_6, self.w_r_6, self.b_conv_6),
                            dim=-1),
            torch.unsqueeze(conv_step(nodes, children, self.feature_size, self.w_t_7, self.w_l_7, self.w_r_7, self.b_conv_7),
                            dim=-1)
            # for _ in range(self.num_cov)
        ]
        conv_output = torch.cat(conv, axis=-1)
        pri_capsules = torch.reshape(conv_output, shape=(1,-1,self.cov_size,self.num_cov))
        primary_static_caps = vts_routing(pri_capsules,self.top_a, self.top_b,self.caps1_num_caps,self.caps1_num_dims)
        ...
        codeCaps = dynamic_routing(dr_shape, primary_static_caps,self.dr_w,self.dr_b, num_outputs=self.caps1_out_caps, num_dims=self.caps1_out_dims)
        codeCaps = torch.squeeze(codeCaps, axis=1)
        v_length = torch.sqrt(reduce_sum(torch.square(codeCaps), axis=2, keepdims=True) + 1e-9)
        out = torch.reshape(v_length, (-1, self.label_size))
