...
class Encoder(nn.Module):
    ...
    def forward(self, content, mask, paths, path_map, r_paths_, r_path_idx):
        ...
        if self.absolute_path:
            if r_path_idx.shape[0] != r_paths_.shape[0]:
                r_path_idx = r_path_idx.repeat(r_paths_.shape[0], 1)
        ...
        if content.shape[0] != path_map.shape[0]:
            path_map = path_map.repeat(content.shape[0], 1, 1)
