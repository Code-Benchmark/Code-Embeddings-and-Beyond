...
class Trainer:
    ...
    def __init__(self, args, model, train_data, valid_data, test_data, valid_infer_data=None, test_infer_data=None):
        self.args = args
        ...
        self.optim = torch.optim.Adamax(self.model.parameters(), lr=self.args.lr)
        ...
        if self.args.relation_path or self.args.absolute_path:
             print(
                "Total Parameters: {}".format(sum([p.nelement() for _, p in self.model.named_parameters()])))
            # print(
    ...
    def iteration(self, epoch, data_loader, type, train=True):
        ...
        for i, data in data_iter:
            ...
            if train:
                ...
                self.optim.zero_grad()
                out = self.model(data)
                loss = self.criterion(out, data['label'])
                loss.backward()
                self.optim.step()
                _, predicted = torch.max(out.data, 1)
                total_acc += (predicted == data['label']).sum().item()
                ...
                with torch.no_grad():
                    ...
                    _, predicted = torch.max(out.data, 1)
                    total_acc += (predicted == data['label']).sum().item()
                    total += self.args.val_batch_size
            avg_loss += loss.item()
        avg_loss = avg_loss / len(data_iter)
        avg_acc = total_acc / total
        print("EP%d_%s, avg_loss=%f, avg_acc=%f " % (epoch, str_code, avg_loss, avg_acc), file=self.writer, flush=True)
